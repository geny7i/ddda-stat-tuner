...
for (const width of [1280, 390]) {
  test(`${width}px: 使い方・免責事項を往復しても種別・体格・経路・帯・注目項目を保持する`, async ({
    page,
  }) => {
    await page.setViewportSize({ width, height: 844 });
    await page.goto("./#/");
    await page.getByRole("radio", { name: "ポーン", exact: true }).check();
...
test.describe("狭い画面のナビゲーション", () => {
  ...
  test("すべての主要画面へ移動できる", async ({ page }) => {
    ...
    await expect(nav.getByRole("link", { name: "チャート" })).toHaveCount(0);
    await nav.getByRole("link", { name: "使い方" }).click();
    await expect(
      page.getByRole("heading", { name: "使い方", level: 1 }),
    ).toBeVisible();
    ...
    await nav.getByRole("link", { name: "育成計画" }).click();
    await expect(
      page.getByRole("heading", { name: "育成計画", level: 1 }),
    ).toBeVisible();
  });
});
